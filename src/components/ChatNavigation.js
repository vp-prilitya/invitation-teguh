function ChatNavigation({ open, onClick }) {
  return (
    <div
      onClick={onClick}
      className={` ${
        open ? "right-6" : "-right-16"
      }  fixed bg-blue-700  bottom-9 z-30 rounded-md p-1 shadow-xl shadow-gray-500 hover:cursor-pointer hover:bg-blue-800 duration-700`}
    >
      <svg
        fill="currentColor"
        viewBox="0 0 20 20"
      ...
      <p className="text-white text-xs text-center">34</p>
    </div>
  );
}
